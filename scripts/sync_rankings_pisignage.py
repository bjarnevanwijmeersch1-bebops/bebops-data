#!/usr/bin/env python3
"""
Sync ranking pages to PiSignage via API.
Creates weblink assets for each ranking and manages a Rankings playlist.
"""
...
# GitHub Pages base URL for ranking pages
...
RANKINGS_HTML_DIR = PROJECT_ROOT / "frames" / "rankings"

# Playlist name for rankings
RANKINGS_PLAYLIST = "Rankings"

# Prefix for ranking link assets (to identify them)
RANKING_ASSET_PREFIX = "ranking_"

# Duration per ranking slide (seconds)
SLIDE_DURATION = 10
...
def create_weblink(token: str, name: str, url: str, duration: int = 10) -> dict:
...
def update_playlist_assets(token: str, playlist_name: str, asset_filenames: list, duration: int = 10) -> dict:
...
def get_local_ranking_pages() -> list:
    """Get list of local ranking HTML pages."""
    if not RANKINGS_HTML_DIR.exists():
        return []

    pages = []
    for f in sorted(RANKINGS_HTML_DIR.iterdir()):
        if f.is_file() and f.suffix == ".html":
            pages.append({
                "filename": f.name,
                "stem": f.stem,
                "url": f"{PAGES_BASE_URL}/frames/rankings/{f.name}"
            })
    return pages
...
def main():
    """Main sync function."""
    print("Starting PiSignage rankings sync...")
    ...
    # Get local ranking pages
    local_rankings = get_local_ranking_pages()
    print(f"Found {len(local_rankings)} local ranking pages")

    if not local_rankings:
        print("No ranking pages found, skipping sync")
        return True
    ...
    # Find existing ranking assets (by prefix)
    existing_ranking_assets = {
        name: info for name, info in existing_assets.items()
        if name.startswith(RANKING_ASSET_PREFIX)
    }
    print(f"Found {len(existing_ranking_assets)} existing ranking assets")

    # Determine what needs to be added/removed
    local_stems = {s["stem"] for s in local_rankings}
    existing_stems = {name.replace(".link", "").replace(".html", "") for name in existing_ranking_assets.keys()}

    to_add = local_stems - existing_stems
    to_remove = existing_stems - local_stems

    print(f"Rankings to add: {len(to_add)}")
    print(f"Rankings to remove: {len(to_remove)}")

    # Remove old ranking assets
    ...
    # Add new ranking weblinks
    for ranking in local_rankings:
        if ranking["stem"] in to_add:
            # Add trailing dot - PiSignage appends "link" to make ".link"
            link_name = f"{RANKING_ASSET_PREFIX}{ranking['stem']}."
            create_weblink(token, link_name, ranking["url"], SLIDE_DURATION)

    # Ensure playlist exists
    playlist = get_playlist(token, RANKINGS_PLAYLIST)
    if not playlist:
        create_playlist(token, RANKINGS_PLAYLIST)

    # Build list of all ranking asset filenames for playlist
    ...
    ranking_asset_names = sorted([
        name for name in updated_assets.keys()
        if name.startswith(RANKING_ASSET_PREFIX)
    ])
    print(f"Ranking assets found: {ranking_asset_names}")

    # Update playlist with all ranking assets
    if ranking_asset_names:
        update_playlist_assets(token, RANKINGS_PLAYLIST, ranking_asset_names, SLIDE_DURATION)

        # Fetch playlist again to verify
        print("\nVerifying playlist contents...")
        get_playlist(token, RANKINGS_PLAYLIST)
    else:
        print("Warning: No ranking assets found to add to playlist")

    print(f"\nSync complete! {len(ranking_asset_names)} rankings in playlist")
    return True
